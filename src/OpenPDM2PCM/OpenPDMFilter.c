...
int64_t filter_table_48(uint8_t *data, uint8_t decimation_index, TPDMFilter_InitStruct *Param) {
  int64_t sum = 0;
  int offset = decimation_index * (48 / 8);
...
// Use your generated coefficients here:
static const int16_t fir_48[] = {
  -3,     -3,     -2,     -1,      0,      1,      2,      3,      5,      7,
   9,     11,     14,     18,     22,     26,     31,     37,     43,     50,
  58,     66,     75,     85,     96,    107,    119,    132,    145,    159,
 174,    189,    205,    222,    238,    256,    274,    292,    310,    329,
 347,    366,    385,    403,    421,    439,    457,    474,    491,    507,
 523,    537,    551,    564,    576,    587,    597,    606,    613,    620,
 625,    629,    631,    633,    633,    631,    629,    625,    620,    613,
 606,    597,    587,    576,    564,    551,    537,    523,    507,    491,
 474,    457,    439,    421,    403,    385,    366,    347,    329,    310,
 292,    274,    256,    238,    222,    205,    189,    174,    159,    145,
 132,    119,    107,     96,     85,     75,     66,     58,     50,     43,
  37,     31,     26,     22,     18,     14,     11,      9,      7,      5,
   3,      2,      1,      0,     -1,     -2,     -3,     -3
};

#define FIR_TAPS_48 (sizeof(fir_48) / sizeof(fir_48[0]))


void Open_PDM_Filter_48(uint8_t* data, uint16_t* dataOut, uint16_t volume, TPDMFilter_InitStruct *Param)
{
    int64_t acc;
    int pdm_index = 0;
    int samples = Param->Fs / 1000;
    int scale = 32768;  // Scale factor to normalise back to int16 range

    for (int i = 0; i < samples; i++) {
        acc = 0;

        for (int j = 0; j < FIR_TAPS_48; j++) {
            uint8_t bit = (data[(pdm_index + j) / 8] >> (7 - ((pdm_index + j) % 8))) & 0x1;
            acc += (bit ? 1 : -1) * fir_48[j];
        }

        acc = (acc * volume) / scale;

        if (acc > 32767) acc = 32767;
        if (acc < -32768) acc = -32768;

        dataOut[i] = (int16_t)acc;
        pdm_index += 48;
    }
}

// #define FIR_TAPS_48 48

// // Boxcar FIR filter — average all 48 bits
// static const int16_t fir_48_int[FIR_TAPS_48] = {
//     [0 ... FIR_TAPS_48-1] = 682  // 32768 / 48 ≈ 682
// };
...
//         for (int j = 0; j < FIR_TAPS_48; j++) {
//             uint8_t byte = data[(pdm_index + j) / 8];
//             uint8_t bit_pos = 7 - ((pdm_index + j) % 8);
//             uint8_t bit = (byte >> bit_pos) & 0x1;

//             acc += (bit ? 1 : -1) * fir_48_int[j];
//         }

//         acc = (acc * volume) / 32768;
