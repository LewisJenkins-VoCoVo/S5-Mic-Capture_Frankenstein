...
#define PDM_DECIMATION       64 // 64 by default. 48 not supported, but a custom filter will resolve this.
...
int pdm_microphone_init(const struct pdm_microphone_config* config) {
    ...
    // Switch these to adjust the clock.
    //float clk_div = clock_get_hz(clk_sys) / (config->sample_rate * PDM_DECIMATION * 4.0); // 1MHz (64 Decimation)
    float clk_div = clock_get_hz(clk_sys) / (768000 * 4.0); // 768kHz (48 Decimation - Implementation unfinished)
    //
...
#elif PDM_DECIMATION == 48
        Open_PDM_Filter_48(in, out, pdm_mic.filter_volume, &pdm_mic.filter);
#else
